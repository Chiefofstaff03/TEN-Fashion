...
export const ProductCard = ({ data }) => {
  ...
  //     try {
  //         const response = await axios.post('https://ten-fashion-m1gv.onrender.com/cartItem', { productId, quantity, productTitle, price, images })
          ...
          <div className="px-5 pb-5">
            ...
            <div className="flex items-center mt-2.5 mb-5">
              <div className="flex items-center space-x-1 rtl:space-x-reverse">
                ...
                {[1, 2, 3, 4, 5].map((star) => (
                  <svg
                    key={star}
                    className={`w-4 h-4 ${
                      star <= datas.rating ? "text-yellow-300" : "text-gray-200"
                    }`}
                    aria-hidden="true"
              ...
              <span className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ms-3">
                {datas.rating ? datas.rating.toFixed(1) : "No rating"}{" "}
                {/* Display the rating */}
              </span>
            </div>
